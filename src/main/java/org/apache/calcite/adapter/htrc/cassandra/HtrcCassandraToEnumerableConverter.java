...
public class HtrcCassandraToEnumerableConverter extends ConverterImpl implements EnumerableRel {
	...
	protected HtrcCassandraToEnumerableConverter(RelOptCluster cluster, RelTraitSet traits, RelNode input) {
		super(cluster, ConventionTraitDef.INSTANCE, traits, input);
	}
	
	  @Override public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
		    return new HtrcCassandraToEnumerableConverter(
	...
	public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
		...
		// Generates a call to "query" with the appropriate fields and
		// predicates
		final BlockBuilder list = new BlockBuilder();
		final HtrcCassandraRel.Implementor cassandraImplementor = new HtrcCassandraRel.Implementor();
		System.out.println("*** selected fields: " + cassandraImplementor.selectFields.keySet());
		RelNode inputNode = getInput();
		System.out.println(inputNode.getClass().getCanonicalName());
		cassandraImplementor.visitChild(0, inputNode);
		System.out.println("*** selected fields: " + cassandraImplementor.selectFields.keySet());
		...
		final Expression table = list.append("table",
				cassandraImplementor.table.getExpression(HtrcCassandraTranslatableTable.HtrcCassandraQueryable.class));
		...
		Expression enumerable = list.append("enumerable",
				Expressions.call(table, HtrcCassandraMethod.CASSANDRA_QUERYABLE_QUERY.method, fields, selectFields,
						predicates, order, offset, fetch));
	...
	/**
	 * E.g. {@code constantArrayList("x", "y")} returns "Arrays.asList('x',
	 * 'y')".
	 */
